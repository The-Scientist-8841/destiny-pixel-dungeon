...
public enum Talent {
	...
	//Mage T1
	EMPOWERING_MEAL(32,4), SCHOLARS_INTUITION(33,4), LINGERING_MAGIC(34,4), BACKUP_BARRIER(35,4), SPELLCASTERS_INGENUITY(59,4), MAGES_JOURNEY(60,4),
	//Mage T2
	...
	//Warlock T3
	SOUL_EATER(46, 4), SOUL_SIPHON(47, 4), NECROMANCERS_MINIONS(48, 4), REJUVENATION(202,4),
	//Global T4
	MAGES_PLIGHT(63,4),
	//Elemental Blast T4
	...
	//Assassin T3
	ENHANCED_LETHALITY(75, 4), ASSASSINS_REACH(76, 4), BOUNTY_HUNTER(77, 4), SILENT_TAKEDOWN(203,4),
	//Freerunner T3
	EVASIVE_ARMOR(78, 4), PROJECTILE_MOMENTUM(79, 4), SPEEDY_STEALTH(80, 4), SPEEDY_RECOVERY(204, 4),
	//Global T4
	ROGUES_PLIGHT(95,4),
	//Smoke Bomb T4
	...
	//Duelist T1
	STRENGTHENING_MEAL(128,4), ADVENTURERS_INTUITION(129,4), PATIENT_STRIKE(130,4), AGGRESSIVE_BARRIER(131,4), ADVENTURING_GEAR(155,4), DUELISTS_JOURNEY(156,4),
	//Duelist T2
	...
	//Duelist T3
	PRECISE_ASSAULT(137, 4), DEADLY_FOLLOWUP(138, 4), DUELISTS_STRUGGLE(158,4),
	//Champion T3
	...
	public static int[] tierLevelThresholds = new int[]{0, 2, 26, 54, 82, 102};
	...
	public static void onPotionUsed( Hero hero, int cell, float factor ){
		...
		if (hero.hasTalent(LIQUID_SCAVENGING)) {
			if (hero.heroClass == HeroClass.ARTIFICER) {
				Dungeon.materials += Math.round(factor*(1 + hero.pointsInTalent(Talent.LIQUID_SCAVENGING)/2 + Random.Int(1 - hero.pointsInTalent(Talent.LIQUID_SCAVENGING) % 2)));
			} else {
				Dungeon.energy += (Math.round(factor * hero.pointsInTalent(LIQUID_SCAVENGING)));
			}
	...
	public static void initClassTalents( HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents, LinkedHashMap<Talent, Talent> replacements ){
		...
		switch (cls){
			...
			case HUNTRESS:
				...
				break;
			case DUELIST:
				Collections.addAll(tierTalents, STRENGTHENING_MEAL, ADVENTURERS_INTUITION, PATIENT_STRIKE, AGGRESSIVE_BARRIER, ADVENTURING_GEAR, DUELISTS_JOURNEY);
				break;
			case CLERIC:
		...
		switch (cls){
			case WARRIOR: default:
				...
				break;
			case MAGE:
				Collections.addAll(tierTalents, ENERGIZING_MEAL, INSCRIBED_POWER, WAND_PRESERVATION, ARCANE_VISION, SHIELD_BATTERY, EMPOWERING_BLESSING, MAGES_TRIAL);
				break;
			case ROGUE:
				Collections.addAll(tierTalents, MYSTICAL_MEAL, INSCRIBED_STEALTH, WIDE_SEARCH, SILENT_STEPS, ROGUES_FORESIGHT, EVASIVE_BLESSING, ROGUES_TRIAL);
				break;
			case HUNTRESS:
				Collections.addAll(tierTalents, INVIGORATING_MEAL, LIQUID_NATURE, REJUVENATING_STEPS, HEIGHTENED_SENSES, DURABLE_PROJECTILES, KEEN_BLESSING, HUNTRESS_TRIAL);
				break;
			case DUELIST:
				Collections.addAll(tierTalents, FOCUSED_MEAL, LIQUID_AGILITY, WEAPON_RECHARGING, LETHAL_HASTE, SWIFT_EQUIP, FERVENT_BLESSING, DUELISTS_TRIAL);
				break;
			case CLERIC:
				...
				break;
			case ARTIFICER:
				Collections.addAll(tierTalents, INSPIRING_MEAL, LIQUID_SCAVENGING, TOOLS_OF_THE_TRADE, TRUSTY_SIDEARM, AUTO_TURRET, INGENIOUS_BLESSING, ARTIFICERS_TRIAL);
				break;
		}
		for (Talent talent : tierTalents){
			if (replacements.containsKey(talent)){
	...
	public static void initSubclassTalents( HeroSubClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents ){
		...
		switch (cls){
			case BERSERKER: default:
				Collections.addAll(tierTalents, ENDLESS_RAGE, DEATHLESS_FURY, ENRAGED_CATALYST, QUICK_TO_ANGER);
				break;
			case GLADIATOR:
			...
			case BATTLEMAGE:
				...
				break;
			case WARLOCK:
				Collections.addAll(tierTalents, SOUL_EATER, SOUL_SIPHON, NECROMANCERS_MINIONS, REJUVENATION);
				break;
			case ASSASSIN:
				Collections.addAll(tierTalents, ENHANCED_LETHALITY, ASSASSINS_REACH, BOUNTY_HUNTER, SILENT_TAKEDOWN);
				break;
			case FREERUNNER:
				Collections.addAll(tierTalents, EVASIVE_ARMOR, PROJECTILE_MOMENTUM, SPEEDY_STEALTH, SPEEDY_RECOVERY);
				break;
			case SNIPER:
				Collections.addAll(tierTalents, FARSIGHT, SHARED_ENCHANTMENT, SHARED_UPGRADES, MARKED_FOR_DEATH);
				break;
			case WARDEN:
				Collections.addAll(tierTalents, DURABLE_TIPS, BARKSKIN, SHIELDING_DEW, CIRCLE_OF_LIFE);
				break;
			case CHAMPION:
