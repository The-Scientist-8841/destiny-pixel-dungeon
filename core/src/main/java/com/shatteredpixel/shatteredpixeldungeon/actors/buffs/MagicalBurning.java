...
import com.shatteredpixel.shatteredpixeldungeon.actors.blobs.MagicalFire;
...
public class MagicalBurning extends Buff implements Hero.Doom {
	...
	public boolean act() {
		...
		if (target.isAlive() && !target.isImmune(getClass())) {
			...
			if (target instanceof Hero
				...
				//at 6+ turns, there is a (turns-6)/5 chance an item burns
				if (Random.Int(5) < (burnIncrement - 6)){
					burnIncrement = 0;
		...
		if (Dungeon.level.flamable[target.pos] && Blob.volumeAt(target.pos, MagicalFire.class) == 0) {
			GameScene.add( Blob.seed( target.pos, 4, MagicalFire.class ) );
		}
	...
	public void reignite( Char ch, float duration ) {
		if (ch.isImmune(MagicalBurning.class)){
			if (ch.glyphLevel(Brimstone.class) >= 0){
				//generate avg of 1 shield per turn per 50% boost, to a max of 4x boost
				float shieldChance = 4*(Armor.Glyph.genericProcChanceMultiplier(ch) - 1f);
	...
	public void fx(boolean on) {
		if (on) target.sprite.add(CharSprite.State.MAGICAL_BURNING);
		else target.sprite.remove(CharSprite.State.MAGICAL_BURNING);
	}
