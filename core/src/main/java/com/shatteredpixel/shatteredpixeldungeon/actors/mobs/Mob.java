...
public abstract class Mob extends Char {
	...
	public void die( Object cause ) {
		...
		if (alignment == Alignment.ENEMY){
			...
			if (cause == Dungeon.hero || cause instanceof Weapon || cause instanceof Weapon.Enchantment){
				...
				if (Dungeon.hero.hasTalent(Talent.ROGUES_PLIGHT) && Random.Float() < 0.005f + 0.005f*Dungeon.hero.pointsInTalent(Talent.ROGUES_PLIGHT)) {
					evilBonus = true;
					ScrollOfTerror reward = new ScrollOfTerror();
					if (!reward.collect()) {
						Dungeon.level.drop(reward, Dungeon.hero.pos);
					}
				}

				if (Dungeon.hero.hasTalent(Talent.HUNTRESS_PLIGHT) && Random.Float() < 0.005f + 0.005f*Dungeon.hero.pointsInTalent(Talent.HUNTRESS_PLIGHT)) {
					evilBonus = true;
					ScrollOfLullaby reward = new ScrollOfLullaby();
					if (!reward.collect()) {
						Dungeon.level.drop(reward, Dungeon.hero.pos);
					}
				}
	...
	public void rollToDropLoot(){
		//if (Dungeon.hero.lvl > maxLvl + 2) return; No! More loot!
		...
		if (Dungeon.hero.belongings.getItem(EvilBook.class) != null) {
			...
			if (Dungeon.hero.hasTalent(Talent.DUELISTS_STRUGGLE)) {
				if ((Dungeon.hero.pointsInTalent(Talent.DUELISTS_STRUGGLE) == 1) && (evil_bonus == null)) {
					if (Random.Float() < 0.01f) {
						Item bonus = Generator.randomUsingDefaults(Generator.Category.POTION);
				...
				} else if (Dungeon.hero.pointsInTalent(Talent.DUELISTS_STRUGGLE) > 1) {
					if (Random.Float() < 0.005f*Dungeon.hero.pointsInTalent(Talent.DUELISTS_STRUGGLE)) {
						Item reward = Generator.randomUsingDefaults(Generator.Category.POTION);
						Dungeon.level.drop(reward, pos).sprite.drop();
			...
			if (Dungeon.hero.hasTalent(Talent.CLERICS_STRUGGLE)) {
				if ((Dungeon.hero.pointsInTalent(Talent.CLERICS_STRUGGLE) == 1) && (evil_bonus == null)) {
					if (Random.Float() < 0.01f) {
						Item bonus = Generator.randomUsingDefaults(Generator.Category.SCROLL);
						Dungeon.level.drop(bonus, pos).sprite.drop();
						if (!didFlare) {
							EvilBook.showFlareForBonusDrop(sprite);
							didFlare = true;
				...
				} else if (Dungeon.hero.pointsInTalent(Talent.CLERICS_STRUGGLE) > 1) {
					if (Random.Float() < 0.005f*Dungeon.hero.pointsInTalent(Talent.CLERICS_STRUGGLE)) {
						Item reward = Generator.randomUsingDefaults(Generator.Category.SCROLL);
						Dungeon.level.drop(reward, pos).sprite.drop();
			...
			if (Dungeon.hero.subClass == HeroSubClass.NONE) {
				...
				WandOfWarding debugThing5 = new WandOfWarding();
				debugThing5.identify();
				debugThing5.cursed = false;
				debugThing5.collect();
			}
		}
