	...
	public void create() {
		...
		switch (lastIDX){
			...
			case 3:
				...
				break;
			case 4:
				WndJournal.ToolboxCraftingTab toolboxCrafting = new WndJournal.ToolboxCraftingTab();
				add(toolboxCrafting);
				toolboxCrafting.setRect(panel.x + panel.marginLeft(),
						panel.y + panel.marginTop(),
						panel.width() - panel.marginHor(),
						panel.height() - panel.marginVer());
				break;
		...
		StyledButton btnToolboxCrafting =  new StyledButton(Chrome.Type.GREY_BUTTON_TR, ""){
			@Override
			protected void onClick() {
				if (lastIDX != 4) {
					lastIDX = 4;
		...
		btnToolboxCrafting.icon(new ItemSprite(ItemSpriteSheet.ARTIFACT_TOOLBOX));
		btnToolboxCrafting.setRect(btnAlchemy.right()-2, btnBadges.top(), pw/5f + 1.5f, lastIDX == 4 ? 25 : 20);
		align(btnToolboxCrafting);
		if (lastIDX != 4) btnToolboxCrafting.icon().brightness(0.6f);
		addToBack(btnToolboxCrafting);
